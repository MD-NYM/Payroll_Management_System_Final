...
export class CrudService {
  ...
  getAttendanceList(employeeId: number, payrollPeriod: string): Observable<AppResponse> {
    const url = `${this.config.apiUrl}/attendance/list`;
    return this.http.get<AppResponse>(url);
  }

  calculateSalary(year: number, month: number): Observable<AppResponse> {
    const url = `${this.config.apiUrl}/salary/calculate-salary/${year}/${month}`;
    return this.http.get<AppResponse>(url);
  }
}
