...
@Repository
public interface SalaryRepository extends JpaRepository<Salary, Long>, CrudDao<Salary, Long> {

    @Modifying
    @Query(value = "UPDATE Salary e " +
            "SET e.active = false, e.modifiedBy = :modifiedBy, e.modifiedDate = :modifiedDate " +
            "where e.id = :id")
    int softDeleteById(@Param("id") Long id, @Param("modifiedBy") String modifiedBy,
            @Param("modifiedDate") LocalDateTime modifiedDate);


    List<Salary> findByEmployeeAndActiveTrue(Employee employee);
